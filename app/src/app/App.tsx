...
import { AethrionisFirstRun } from "@/features/aethrionis/ui/AethrionisFirstRun";
...
// Animated AETHRIONIS Studio mark for the loading gates. The static BuzzMark renders in
...
// Cold boot: the mark inside two counter-turning rings, with the name
// resolving under it. The motion comes from the mark's own plinth rather than
// from a borrowed spinner — a spinner says "waiting", and what a splash should
// say in the second before the application can speak is "this is the thing you
// opened". The old "Setting up your community" text stays as an sr-only
// caption, because a screen reader needs the sentence, not the geometry.
function AppLoadingGate() {
  return (
    ...
    >
      ...
      <div className="aethrionis-boot">
        <div className="aethrionis-boot__halo">
          <div className="aethrionis-boot__ring aethrionis-boot__ring--outer" />
          <div className="aethrionis-boot__ring aethrionis-boot__ring--inner" />
          <AethrionisAppmark className="aethrionis-boot__mark" />
        </div>
        <div className="flex flex-col items-center gap-2 text-center">
          <span className="aethrionis-boot__word">AETHRIONIS</span>
          <span className="aethrionis-boot__sub">Studio</span>
        </div>
      </div>
    </div>
  );
...
function CommunityApp({
  ...
  if (!transaction) {
    if (community.needsSetup) {
      ...
      // AETHRIONIS's workspace exists before the application is installed, so
      // first run connects to it rather than asking which one to join.
      appContent = (
        <AethrionisFirstRun
