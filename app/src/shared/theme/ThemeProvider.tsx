...
/**
 * The AETHRIONIS Studio themes ship with a fixed neutral accent (the GitHub black/white
 * foreground) rather than a user-selectable accent color. When a AETHRIONIS Studio theme is
...
/**
 * Resolve the accent to actually apply for a theme: AETHRIONIS Studio themes are pinned to
...
function applyBuzzSidebar(themeName: string) {
  ...
  if (isBuzzTheme(themeName)) {
    ...
    // Keep the concrete AETHRIONIS Studio variant on the root as well as the generic
...
function applyCachedVars(): string | null {
  try {
    ...
    // Pin AETHRIONIS Studio themes to the neutral accent here too, matching applyTheme.
    // Otherwise a cached AETHRIONIS Studio theme + non-neutral stored accent flashes the
...
async function applyTheme(name: SyntaxThemeName): Promise<{
  ...
  // new theme for a frame — the flicker seen when switching to AETHRIONIS Studio. AETHRIONIS Studio
...
export function ThemeProvider({
  ...
  const [followSystem, setFollowSystemState] = useState<boolean>(() => {
    ...
    // Fresh profiles (no saved theme) default to System mode so the AETHRIONIS Studio
  ...
  // The stronger selected-row treatment belongs exclusively to AETHRIONIS Studio. Keep
  // the saved preference so it is restored when the user returns to AETHRIONIS Studio,
