...
async function createSegmenter(): Promise<SegmenterHandle> {
  // Background removal needs a wasm bundle from jsDelivr and a model from
  // Google. Both are fetched the first time an operator records an avatar, and
  // both are the only outbound requests this application makes that are not to
  // a relay the operator chose. AETHRIONIS does not make them.
  //
  // The capture still works; `backgroundRemoved` comes back false, which is
  // the path upstream already has for a model that would not load, so nothing
  // downstream needs to learn a new case. Set
  // VITE_AETHRION_AVATAR_SEGMENTATION=1 to fetch them anyway.
  if (import.meta.env.VITE_AETHRION_AVATAR_SEGMENTATION !== "1") {
    throw new Error(
      "avatar background removal is off: it would fetch a model from a third party",
    );
  }
