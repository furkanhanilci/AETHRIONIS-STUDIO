import { Activity, Bot, Folders, Inbox, Wrench, Zap } from "lucide-react";
...
export function AppSidebarPrimaryMenu({
  ...
  const dumeActive = useRouterState({
    select: (state) => state.location.pathname === "/dume",
  });
  return (
    <>
      <SidebarHeader
      ...
      >
        <SidebarMenu className="sidebar-primary-menu pb-2">
          ...
          {/* Commissioning is a Link rather than another onSelect callback:
              the view union, the pathname map and four prop chains all exist to
              carry the callbacks upstream defined, and threading a fork's own
              screen through them makes every future merge harder for no gain a
              reader can see. */}
          <SidebarMenuItem>
            <SidebarMenuButton
              asChild
              data-testid="open-dume-view"
              isActive={dumeActive}
              tooltip="Commissioning"
            >
              <Link to="/dume">
                <Wrench className="h-4 w-4" />
                <SidebarMenuLabel>Commissioning</SidebarMenuLabel>
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
          <SidebarMenuItem>
