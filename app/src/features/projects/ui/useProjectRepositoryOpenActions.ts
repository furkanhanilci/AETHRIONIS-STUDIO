  ...
  const handleOpenLocalRepository = React.useCallback(async () => {
    ...
    if (!localRepositoryPath || !repository || !cloneUrl) {
      toast.error("Couldn’t open repository folder", {
        description: "AETHRIONIS Studio could not find this repository’s local checkout.",
      });
      return;
    }
    try {
      ...
      toast.error("Couldn’t open repository folder", {
        description: "AETHRIONIS Studio could not open this checkout in your file browser.",
      });
    }
