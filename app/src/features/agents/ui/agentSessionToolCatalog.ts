...
export function getBuzzToolInfo(title: string): BuzzToolInfo | null {
  ...
  if (name.includes("workflow") || name === "approve_step") {
    return {
      icon: Workflow,
      label: isRead
        ? "Reads workflow state from AETHRIONIS Studio."
        : "Updates workflow state in AETHRIONIS Studio.",
  ...
  if (
    ...
    return {
      icon: Hash,
      label: isRead
        ? "Reads channel context from the AETHRIONIS Studio relay."
        : "Changes channel state in the AETHRIONIS Studio relay.",
      tone: isWrite ? "write" : "read",
    };
  }
  if (
    ...
    return {
      icon: Users,
      label: isRead
        ? "Reads AETHRIONIS Studio identity or presence data."
        : "Updates AETHRIONIS Studio identity or membership data.",
  ...
  if (name.includes("search") || name === "get_feed") {
    return {
      icon: Search,
      label: "Searches relay-visible AETHRIONIS Studio history.",
      tone: "read",
    };
  }
  if (
    ...
    return {
      icon: Send,
      label: "Publishes relay-visible AETHRIONIS Studio activity.",
      tone: "write",
    };
  }
  ...
  return {
    icon: MessageSquare,
    label: isRead ? "Reads from AETHRIONIS Studio." : "Writes to AETHRIONIS Studio.",
    tone: isWrite ? "write" : "read",
  };
}
