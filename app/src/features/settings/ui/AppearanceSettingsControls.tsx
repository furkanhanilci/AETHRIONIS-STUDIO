...
/** AETHRIONIS Studio navigation can use either its production tint or a stronger tab. */
      ...
      <SettingsOptionRow data-testid="conversation-density-row">
        <div className="min-w-0">
          ...
          <p
            ...
            data-settings-subcopy
          >
            Spacing in conversations and Markdown content across AETHRIONIS Studio
          </p>
        </div>
        <SegmentedControl
          size="wide"
...
/**
 * Build the sample thumbnail as an SVG data URL from the AETHRIONIS Studio gradient
 * tokens. Data-URL images cannot resolve CSS variables, so the token values
 * are read from the live stylesheet and baked in per render — if the AETHRIONIS Studio
...
function ThreadLayoutDiagram({ mode }: { mode: ThreadViewMode }) {
  ...
  // AETHRIONIS Studio gradient tokens directly and follows theme.css automatically.
    ...
    <SettingsOptionRow className="items-start">
      <div className="min-w-0">
        ...
        <p
          ...
          data-settings-subcopy
        >
          Choose the highlight color used throughout AETHRIONIS Studio.
        </p>
      </div>
      <div
