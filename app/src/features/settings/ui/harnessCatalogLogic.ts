...
// DoctorSettingsPanel RUNTIME_SORT_PRIORITY so the AETHRIONIS Studio + Goose rows stay
...
export function adapterUpdateWarning(entry: AcpRuntimeCatalogEntry): string {
  if (entry.id === "codex") {
    return (
      "This replaces the machine-wide codex-acp adapter. Older AETHRIONIS Studio " +
