...
// Where a manual download would come from. AETHRIONIS Studio is built from
// this checkout and has no release feed, so there is nowhere to send anyone —
// and the value inherited from upstream sent them to Buzz's releases, which
// would have offered a different application as this one's update.
// AETHRIONIS Studio has no release feed — `updater.endpoints` is empty, so
// this branch is unreachable today. It is kept honest rather than deleted so
// that adding a feed later does not silently re-inherit Buzz's.
const RELEASES_URL = "";
