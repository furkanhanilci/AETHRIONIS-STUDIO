...
fn main() {
    // The frontend is embedded at link time. Without this, a build whose only
    // change is in the interface finishes green and ships the previous
    // interface: cargo sees no Rust source newer than the binary and skips the
    // relink, and `cargo tauri build` has already rebuilt dist by then, so the
    // log says "Finished" over a stale bundle. Cost us four builds that each
