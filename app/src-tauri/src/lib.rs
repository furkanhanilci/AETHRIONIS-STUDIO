...
mod app_state;
mod archive;
// `mod builderlab;` — removed. It opened the operator's browser at
// app.builderlab.xyz to obtain a hosted relay. AETHRIONIS runs its own; sending
// the operator to a third party for an account they do not need is both
// out of scope and a wider trust boundary than the work requires. The commands
// it registered are gone with it, so nothing can reach it from the front end
// either — a dead command is a clearer failure than a working one nobody meant
// to expose. The file is kept in the tree, unreferenced, so the diff against
...
pub fn run() {
    ...
    let app = app_menu::install(builder)
        ...
        .setup(move |app| {
            ...
            // Speech-to-text and text-to-speech belong to huddles, which
            // AETHRIONIS does not surface: the assimilation matrix puts voice out
            // of scope, and it outranks feature parity. Upstream prefetches
            // both at boot, which on a first run pulls hundreds of megabytes
            // from GitHub and HuggingFace for a feature nothing here opens.
            //
            // The code stays — removing it is a large edit into upstream's own
            // module for no benefit, and it would make every future merge
            // harder. Set AETHRIONIS_SPEECH_MODELS=1 to have it behave as
            // upstream does.
            if std::env::var("AETHRIONIS_SPEECH_MODELS").as_deref() == Ok("1") {
